function delay_bram_impl_init(blk, varargin)

  clog('entering delay_bram_impl_init', {'trace', 'delay_bram_init_debug'});
  ...
  implType      = get_var('implementation', 'defaults', defaults, varargin{:});
  ...
  if strcmp(implType,'Behavioral HDL'), 
    use_rpm = 'off'; 
    use_hdl = 'on';
    implementation = 'Fabric';
  else 
    if strcmp(implType,'DSP48'),
        use_rpm = 'on';
        use_hdl = 'off';
