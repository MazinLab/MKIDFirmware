function prog_fir_fabric_init(blk, varargin)
% By Matt Strader
%
% prog_fir block applies a finite impulse response (FIR) filter
...
clog('Entering prog_fir_fabric_init','trace'); 
disp('prog_fir_fabric_init')
...
clog('prog_fir_fabric_init post same_state','trace'); 
% Checks whether the block selected is correct with this called function.
check_mask_type(blk, 'prog_fir_fabric');
...
for iTap=1:nTaps
    ...
    if iTap < nTaps
        ...
        reuse_block(blk,delayname,'xbsIndex_r4/Delay',...
                'Position',[x+2*offset y-2*offset x+2*offset+w y-2*offset+h],...
                'latency',num2str(nChannels));
    end
    if iTap > 1
...
clog('exiting prog_fir_fabric_init','trace'); 
