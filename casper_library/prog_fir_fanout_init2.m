function prog_fir_fanout_init(blk, varargin)
% By Matt Strader
% Neelay Fruitwala
%
...
for i=1:nFanout
    reuse_block(blk, strcat('coeff_in_dly1', num2str(i)), 'casper_library_delays/pipeline',...
            'Position', [300, 550+100*(i-1), 300+50, 550+100*(i-1)+20], 'latency', '1');
    add_line(blk, 'coeff_in_dly0/1', strcat('coeff_in_dly', num2str(i), '/1'), 'autorouting', 'off')
    for j=1:nFanout
        reuse_block(blk, strcat('coeff_in_dly2', num2str(nFanout*(i-1)+j)), 'casper_library_delays/pipeline',...
                'Position', [400, 550+100*(nFanout*(i-1)+j), 400+50, 550+100*(nFanout*(i-1)+j)+20], 'latency', '1');
...
nRemainingTaps = mod(nTaps, nTapsPerDel);
