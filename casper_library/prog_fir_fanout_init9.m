function prog_fir_fanout_init9(blk, varargin)
% By Matt Strader
% Neelay Fruitwala
%
...
reuse_block(blk,'misc_dly','xbsIndex_r4/Delay',...
            ...
            'latency','7');%3 from mux,demux, 3 from coeff brams
reuse_block(blk,'ch_dly0','casper_library_delays/pipeline',...
            'Position',[200 500 200+50 500+20],...
            'latency','1');
...
reuse_block(blk, 'not_clear', 'xbsIndex_r4/Inverter', ...
      'latency', '2', 'Position', [100 850 100+50 850+50]);
...
bramLatency=7;% delay data enough to demux,address and read from coeff bram block
...
add_line(blk,'ch_mux/1','ch_dly0/1','autorouting','off');
...
for i=1:nFanout
    reuse_block(blk, strcat('coeff_in_dly1', num2str(i)), 'casper_library_delays/pipeline',...
            'Position', [400, 550+100*(i-1), 400+50, 550+100*(i-1)+20], 'latency', '1');
    add_line(blk, 'coeff_in_dly0/1', strcat('coeff_in_dly1', num2str(i), '/1'), 'autorouting', 'off')
    
    reuse_block(blk, strcat('ch_dly1', num2str(i)), 'casper_library_delays/pipeline',...
            'Position', [400, 500+100*(i-1), 400+50, 500+100*(i-1)+20], 'latency', '1');
    add_line(blk, 'ch_dly0/1', strcat('ch_dly1', num2str(i), '/1'), 'autorouting', 'off')
    
    for j=1:nFanout
        reuse_block(blk, strcat('coeff_in_dly2', num2str(nFanout*(i-1)+j)), 'casper_library_delays/pipeline',...
                'Position', [600, 550+100*(nFanout*(i-1)+j), 600+50, 550+100*(nFanout*(i-1)+j)+20], 'latency', '1');
        add_line(blk, strcat('coeff_in_dly1', num2str(i), '/1'), strcat('coeff_in_dly2', num2str(nFanout*(i-1)+j), '/1'),...
                'autorouting', 'off');

        reuse_block(blk, strcat('ch_dly2', num2str(nFanout*(i-1)+j)), 'casper_library_delays/pipeline',...
                'Position', [600, 500+100*(nFanout*(i-1)+j), 600+50, 500+100*(nFanout*(i-1)+j)+20], 'latency', '1');
        add_line(blk, strcat('ch_dly1', num2str(i), '/1'), strcat('ch_dly2', num2str(nFanout*(i-1)+j), '/1'),...
                'autorouting', 'off');
...
for iTap=1:nTaps
    ...
    reuse_block(blk, bramName, 'xbsIndex_r4/Single Port RAM', ...
            'Position',[930 400+iTap*120 930+50 400+iTap*120+70],...
    ...
    add_line(blk,strcat('ch_dly2', num2str(coeffDelNumList(iTap)), '/1'),[bramName,'/1'],'autorouting','off');%addr
